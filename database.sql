...
CREATE VIEW article_view AS SELECT s.teaser_text, s.title_text, s.title_url, s.slug_text, s.instance_id, ai.article_id, ai.story_number, ai.element_type, ai.section_type
    FROM standard as s JOIN article_instances AS ai ON s.instance_id = ai.id
  UNION
    SELECT null, b.title_text, b.title_url, b.slug_text, b.instance_id, ai.article_id, ai.story_number, ai.element_type, ai.section_type
    FROM basic as b JOIN article_instances AS ai ON b.instance_id = ai.id
  UNION
    SELECT null, a.title_text, a.title_url, null, a.instance_id, ai.article_id, ai.story_number, ai.element_type, ai.section_type
